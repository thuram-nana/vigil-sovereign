...
<!-- CLAIM:W14-1-egress --> For target traffic specifically, the signed charter scope and the
categorical protected-domain floor bound every run, and the runtime egress allowlist is installed on
the engage and repeater target-traffic executors (W10-1 #473), not merely available: `engage` sets it
on both its scan and discovery executors and the repeater on its replay executor, and a tool that
declares egress hosts is checked against the allowlist unconditionally by the invoker
(`engine/crucible/framework/v2/agents/tools/invoker.py:128-134`). The host-refusal at the httpx-transport
layer engages under a sovereign tier (defence in depth), while the protected-domain floor inside that
transport runs unconditionally (`engine/crucible/framework/v2/agents/egress_guard.py:260-261`).
...
`vigil up` launched it. <!-- CLAIM:W14-1-tier --> The sovereign runtime env is resolved at bring-up
(`integration/vigil_integration/uiproxy.py:3122`) and **re-resolved at each offense-plane
restart** (W10-2 #474, `integration/vigil_integration/uiproxy.py:1026, 3210`), so a tier
changed in Settings while the UI is already running takes effect on the next offense-plane
restart or a fresh `vigil up`, but not on a run already in flight. An offense process started
any other way falls back to `PERMISSIVE` **silently — fail-open**.
The read-only tier pill on the UI's Governance & Gate Audit screen (`#/governance`) is where
to read the tier actually in force.
