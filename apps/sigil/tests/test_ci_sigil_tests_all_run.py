...
# filename -> reason. See the module docstring for the inclusion bar. EMPTY today: grep-triage found no
# apps/sigil test that needs a ubuntu-CI-absent resource, so nothing is --ignore'd and the whole directory
# runs. Add an entry here IF AND ONLY IF you also add a matching `--ignore=apps/sigil/tests/<file>` to the
# sigil-governor job — the equality assertion below fails on either half alone.
