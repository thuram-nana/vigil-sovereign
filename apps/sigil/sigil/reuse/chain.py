"""Compatibility shim — `sigil.reuse.chain` is now `vigil_core.chain` (the shared integrity core).

Re-exports the FULL module namespace (public API + private helpers such as `_head_payload`, `_GENESIS_PREV`,
`_entry_hash`, `_EVIDENCE_DOMAIN`) so every existing `from ..reuse.chain import X` in SIGIL keeps resolving
