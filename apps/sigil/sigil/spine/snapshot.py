...
Sub-state ↔ bearer (all identity-empty in Slice C):
  ...
  account_cred           {username: role/cred_hash/cred_salt}    right-biased LWW               active-account rebuild
...
class SnapshotState(BaseModel):
    ...
    account_cred: list = []           # [[username, role, cred_hash, cred_salt], ...] — the fields to REBUILD an
    #                                   active `Account` from the seed (issued_at joins from account_issued;
    #                                   state="active"). Carried for every honored-active username (incl. ones
    #                                   later revoked — dropped at read time by account_state, exactly as the scan).
...
def build(records: Iterable[SpineRecord], *, trusted_pubkey: Optional[str],
    ...
    acct_cred: dict[Any, list] = {row[0]: [row[1], row[2], row[3]] for row in s.account_cred} if s else {}
    ...
    for r in records:
        ...
        if sig == _ACCT_SIGNAL:
            ...
            elif ustate == "active" and verify_signed(p, _ACCT_CORE, tp):
                ukey = p.get("username")
                at = as_issued_at(p.get("issued_at"))
    ...
    return SnapshotState(
        ...
        account_cred=[[u, c[0], c[1], c[2]] for u, c in acct_cred.items()],
