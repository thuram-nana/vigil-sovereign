...
| **O9** | "everything… offline-verifiable, forever" | ~~`crucible-blackboard-chain` is registered *not publicly verifiable*~~ → **RESOLVED (T3/T3b):** every engage run now persists an owner-rooted, public-key-only offline-verifiable `spine-head.json`+`spine-chain.json` (universal, not fireteam-only); graph/telemetry remain honestly out of the signed spine | `README.md:157,716` vs `FEATURES.md:775` |
