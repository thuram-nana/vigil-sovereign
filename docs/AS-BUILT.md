...
So the honest form of the claim is: **a pull request cannot merge without those 13 checks green, on a branch up to date with `main` — unless the
...
`gh api repos/thuram-nana/vigil-sovereign/branches/main/protection`. Expect `required_status_checks.contexts` to hold the 13 names above (compare it against `.github/required-status-checks.txt`), `required_status_checks.strict = true`, `enforce_admins.enabled = false`, `allow_force_pushes.enabled = false` and `allow_deletions.enabled = false`.
...
|------|----|-----------|------------|
| **A14 — supply-chain hardening** | #294 | `.github/workflows/supply-chain.yml`, `infra/supply-chain/{sovereign.in,sovereign.lock.txt,image_pins.py,resolve-image-digests.sh}`, `engine/crucible/framework/v2/requirements.lock.txt`, `.trivyignore`, `docs/SUPPLY-CHAIN.md` | Base images **digest-pinned**; **two real hash-locked dependency locks** (one per environment) whose installability is proven in CI; an **SBOM**; and a **blocking CVE gate** — `trivy fs --severity CRITICAL --exit-code 1` over the locks. The gate carries a **negative control** that runs that exact configuration against a known-CRITICAL fixture and hard-fails if it exits 0, so "the gate can fire" is demonstrated rather than asserted. Honest scope is published in `docs/SUPPLY-CHAIN.md` §6 (HIGH is advisory; non-Python ecosystems are scanned, not locked; hashes are not signatures). |
