...
| Live L2 **external** FACT (2026-07-29, re-corroborated 2026-07-30) | `targets/testasp/charter.md` §7 | The byte-identical external run is **DONE and re-corroborated**: a fresh chartered run against the vendor-published `testasp.vulnweb.com` on 2026-07-30 ran live through the full gate chain (every destructive probe **default-denied**, no TTY) and minted **2 oracle-confirmed, certificate-backed findings** onto the signed spine (`.blackboard/store.sqlite`, `--spine`): `boolean_sqli` (`differential_response`, 0.99) + `open_redirect` (`achieved_state`, 0.90). The **signed spine carries these FACTs** (resolving an earlier per-run `telemetry.json` that showed only a refusal — a destructive-edges-denied view, not the confirmed edge plane). Two `request_smuggling` timing detections (CL.TE / TE.TE) were also observed but were **UNCONFIRMED LEADs**, capped at LEAD per audit **A12 (#269)** — timing alone is a hypothesis, not proof, so they were **not** FACTs and were **not** carried as confirmations on the spine. **A12 re_verified_closed (Wave-4.2):** the timing signal is **retired** and `request_smuggling` is **re-promoted to fact_capable** via a DIFFERENTIAL desync proof — a unique per-probe canary VIGIL embeds in the smuggled prefix must be echoed in VIGIL's OWN second-request response on the CONFLICT connection **and** absent from an identical WELL-FORMED control connection (`request_smuggling.differential_desync`, kind `differential_response`; `verify/oracles.py::smuggling_desync_oracle` + the gated raw-socket re-drive `live/smuggling_redrive.py`, re-gated through `validate_action` + DNS-pin). No victim is poisoned (both requests are VIGIL's own on its own sockets); timing is not consulted and a bare mangled-method status does not mint. The honest boundary that remains, bounded and stated: a shared-pool desync needing a real co-tenant victim to observe, an HTTPS origin, or genuine HTTP/2 desync stays a LEAD, never a FACT. The self-contained **3/3 offline re-verify** is demonstrated on-box by the loopback L1 FACT (`AS-BUILT-LIVE.md` §L1); this external run is corroborated by minting through the full gate + a tampered-baseline byte rejected (`[BAD] CLAIM-MISMATCH`). Demonstrated live **and** external. |
...
| **E4 live-fire against a REAL cluster** | #297 | `tools/livefire/k8s_rbac_livefire.{sh,py}` | The K8s branches are **no longer fixture-only**. The harness stands up a real single-node k3s cluster **VIGIL itself creates and owns** (throwaway, loopback-only, destroyed afterwards), plants known-dangerous *and* known-benign RBAC, captures what the real API server returns, and adjudicates those real bytes through the production path (oracle → admission → certificate → offline re-verify). Anonymous→cluster-admin CONFIRMED with an offline-re-verifying certificate; anonymous→view and named-user→cluster-admin correctly **not** confirmed. It **exits non-zero on any deviation** — a live-fire script that only prints is a demo, not a proof — and refuses to conclude anything while the cluster's role controller is still populating built-in roles (a race the harness caught on its own first run). It is now wired as a **scheduled (nightly, non-blocking) CI job** (W11-2 #483) rather than a run by hand; the *required* PR-CI guarantee is narrower and honestly so — the fail-closed-on-missing-tool preflight (`tools/livefire/require_tools.py`) that guards it, proven in the required P5 job. What it does **not** cover is stated in the registry: a scope-gated *enumeration* runner that discovers bindings cluster-wide, and a managed control plane (EKS/GKE/AKS). |

**Live-fire status across the E-series, stated plainly:** E4's Kubernetes branches are exercised by a
live-fire harness now wired as a **scheduled (nightly, non-blocking) CI job** (#297; W11-2 #483) — it was
previously a manual run by hand. The required-CI guarantee is the fail-closed-on-missing-tool preflight
that guards it, not the nightly cluster run itself. **E1, E2, E3 and E5 are fixture-proven, not live-fired** — the runner, admission, certificate and
