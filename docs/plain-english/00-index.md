...
| **Ten proving results across the nine tools with a typed argument builder, and every one of them now passes.** Each requires all five of: the engine built the command; the tool ran against a live target; an engine reader turned what came back into the system's own record; the planted weakness was reported where it was planted; and the same driver stayed silent on a sound control that was demonstrably still being tested. Nine tools and ten results because the **password-guessing tool has one for each of the two kinds of login it can attack** — a browser's own challenge, and a real login form, which is what practically every real service uses and which had never been exercised against one. Three of the nine met the standard at the start of the day; of the other six, three produced output the system had **no way to read at all** and three had **never been executed** at all, the engine's own permission manifest not naming them, so the interface listed them as controllable while every call was refused. Every gap was closed by building the missing part. It is a statement about the instruments on a local range — **not a field record**, nothing those tools report is a fact, and two of the nine cannot be reached by the ordinary route as the production wiring stands today. Since the fourth revision the exercise **runs in the automated build** — a fast slice on every proposed change and the whole of it nightly, where before it ran only when a person started it; it still names any builder that has no proving result, so a new driver cannot arrive unproven and unnoticed | Ch. 1, section 9.8; Ch. 12, Part One |
