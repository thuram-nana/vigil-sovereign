...
| # | Item | Evidence | Size |
|---|---|---|---|
| B-15 | **Every scan launched from the UI silently skips 93% of the check corpus.** `console/actions.py:306` takes `use_library: bool = True`; the body never reads it and the spawned command (`:322-329`) has no `--library`. `DEFAULT_CHECKS` is **11 checks** (`scanner/checks.py:1055-1067`); the library holds ~165 across 23 bug classes — all NoSQL/LDAP/XPath/EL injection, all SSTI engines, all SQLi dialects. One line | verified | 30 m |
| B-16 | **`--library` is off by default in both CLI entry points too** (`scanner/cli.py:102`, `engage.py:1181`, both `store_true`). The `TIMING` oracle therefore never fires in a default run | verified | decision, not build |
...
| B-22 | **`postmortem.run()` double-counts calibrated priors.** `memory/postmortem.py:198-205` calls `_update_priors_from_engagement` with no run-guard. `docs/FEATURES.md:711` documents this honestly and it was never fixed | verified | 0.5 d |
| B-23 | **CSRF, clickjacking and postMessage have no detection at all**, though the constitution lists all three as always-applicable. `csrf` has **no `BUG_CLASS_ORACLES` row**, so even an imported CSRF finding cannot be adjudicated — the repo's own test says so: `knowledge_engine/tests/test_evolve.py:29` ("csrf → NOT oracle-known → coverage gap"). `clickjacking` / `postMessage`: zero occurrences repo-wide | verified | 1–2 w |
...
| B-27 | `verify/reachability.py:34-47` rejects IPv6 targets outright **and its stated reason is now false** — `common/ethics.py:202-276` added `bracket_bare_ipv6` and canonical IPv6 scope matching. Fail-closed, so a capability gap not a hole | verified | 1 d |
...
| B-43 | Insertion coverage (3 no-gap branches still carrying blocking work) | The re-drive probes only `QUERY_VALUE` and `URL_PATH_SEG` — **cookie, urlencoded-body and JSON-body redirect parameters are never probed** | 1–2 w |
