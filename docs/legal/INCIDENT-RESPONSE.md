...
content was eligible to leave (`/PRIVACY.md` §5.1, and §5.1a for which mechanism reaches which process). **Export it in the environment** every offense
...
Work from `/PRIVACY.md` §3 and §4 — they are the inventory. For a host-level compromise, assume the
...
| What left the machine, and when? | usage-attestation ledger, engagement audit logs, the egress table in `/PRIVACY.md` §5 | The tier in force decides whether prompt content was eligible to reach a model provider. Check the tier that was actually set, not the one you intended. |
...
3. **The categories of personal data involved**, from `/PRIVACY.md` §3 — distinguishing operator/user
...
- **The gaps that were already known.** `/PRIVACY.md` §7 lists the honest limits — plaintext at rest
...
- **Export** and seal a sovereignty tier in the environment every offense process inherits — not only
  in `~/.sigil/sigil.env` — so "what left the machine" is a bounded question (`/PRIVACY.md` § 5.1a).
