...
| | |
|---|---|
| **Where** | `<base_dir>/usage-ledger.jsonl`, default `.vigil-live/usage-ledger.jsonl` (`integration/vigil_integration/live/wiring.py:351`) |
...
| **Survives deletion** | **The monotonic anti-back-dating counter survives** (W10-3 #475). <!-- CLAIM:W14-1-anchor --> The monotonic anti-back-dating counter lives host-level at `~/.vigil/attestation/monotonic.counter`, outside any engagement base directory, so `rm -rf .vigil-live` erases the ledger but the monotonic counter survives (`integration/vigil_integration/attestation/anchor.py:48, 126-131`). The live wiring injects that host path (`integration/vigil_integration/live/wiring.py:359`) and additively migrates any legacy in-base `attest-anchor.json` up into it **without ever lowering the floor** (`wiring.py:360`; `anchor.py:134`). A base-dir reset therefore cannot re-attest under a lower counter, and a truncation of a still-present ledger is caught because the host floor stands above the surviving records. Preserve `~/.vigil/attestation/` off-host as evidence alongside the ledger. |
...
| Monotonic attestation floor (usage ledger) | `~/.vigil/attestation/monotonic.counter` — **host-level, outside** the base dir (`integration/vigil_integration/live/wiring.py:359`; `integration/vigil_integration/attestation/anchor.py:48`), so it **survives** removal of the base dir: `rm -rf .vigil-live` deletes the ledger but not the counter, and a rollback cannot re-attest under a lower floor (see §2.2). |
...
| Sovereign spine | **Decide deliberately** — see §2.1 and `/PRIVACY.md` (Retention) | no per-record mechanism exists | The real decision is upstream: what is allowed to enter the ledger at all. |
