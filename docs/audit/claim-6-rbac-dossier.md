...
- **MFA (TOTP) + optional password.** `governor/totp.py` is a stdlib RFC-6238 implementation; the secret is
  ...
  is signed into the grant. A TOTP-enrolled account must present a valid current code at `/api/login`
  (fail-closed, on top of any login method); a stale code is refused. `set_password` adds an *optional* weaker
  salted-scrypt password login; keypair PoP remains the stronger path.
...
| 9 | A viewer is refused an owner-tier offense action | As a `viewer` through `vigil up`, POST an owner-tier offense route (e.g. `/api/terminal/run`, `/api/authority/provision`) → **403** (`role_can` denies `offense_authority`); the hop-assertion is verified but the role lacks the permission. |
| 10 | A keypair PoP login works, and a replay is refused | `enroll_pubkey` a user's Ed25519 key; `GET /api/login/challenge`, sign it, `POST /api/login` (PoP) → session bearer minted; re-send the **same** `{username, challenge, signature}` → **401** (challenge already consumed). |
| 11 | A TOTP second factor is required, and a stale code fails | `enroll_totp` a user (URI shown once); `POST /api/login` **without** a `totp` code → refused; with a **stale/old-window** code → **401**; with the current code → OK. |
...
*Reproduction commands for rows 1–13 are in [`docs/pilot/claim-6-pilot-runbook.md`](../pilot/claim-6-pilot-runbook.md).*
