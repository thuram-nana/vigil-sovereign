...
| `vigil remediate-campaign` | Batch remediation over EVERY confirmed fact on a codebase's signed spine (`--from-spine <slug> --target-repo <repo>`): ranks + dedupes, runs the gated deep-fix per finding on a disposable clone (never a PR), optionally mints a Remediation Attestation per verified fix (`--attest --attest-key`), and writes a SIGNED, hash-chained remediation ledger. Exit 0 iff no finding ended FAILED. |
