...
**How it works.** The `OracleKind` enum holds **25 kinds** in all — the **15 offensive** oracles
that form the frozen benchmark authority, **plus 10 additive kinds held OUT of the frozen
...
RS256→HS256 confusion, keyed on a `jwt_token` context no benchmark finding carries), **1**
...
shape, keyed on a `saml_xml` context), and **1** `CLOUD_POSTURE` (an offline achieved-state cloud/CSPM
...
path-traversal blocks add **no** new kind — they reuse `EVALUATION` and `SIDE_EFFECT` below (in-band
XXE is emitted only as a **lead**, never an inline block — a single inline exchange cannot soundly
prove it). Every
