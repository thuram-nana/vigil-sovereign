...
- Fresh per-run challenges make replayed or hallucinated "proofs" structurally impossible — for the bug
  classes that map to a challenge kind (`challenge_oracle.BUG_CLASS_CHALLENGE`: sqli/xss/ssti/reflection,
  idor/lfi/info_leak, ssrf/rce/xxe, memory_safety). A class outside that map gets no challenge, so it
  **does not** carry this property and rests on deterministic oracle re-execution instead.
...
| 🆕 **2026-07 hardening program (merged & CI-green; see [`docs/AS-BUILT.md`](docs/AS-BUILT.md) §"What's assured now")** | Every remaining software-completable pending/scaffolded item is now implemented and merged behind the required CI checks (**14** of them today, listed in [`docs/AS-BUILT.md`](docs/AS-BUILT.md) §"What's assured now"): per-action approval as the default + the Strix shell gated by default (#178); per-finding **how-to-verify on every surface** + the per-session graph backend + `dossier --session` + Inbox/assurance/feed UI (#179); the **live-fire honesty reconciliation** (loopback FACT 3/3 offline; external testasp re-corroborated live with 2 spine-signed FACTs — `boolean_sqli` + `open_redirect`; two `request_smuggling` timing detections stay UNCONFIRMED LEADs per audit A12/#269, not FACTs) (#180); the **proof-carrying-finding open standard + a VIGIL-free standalone verifier** and **attack-path/chokepoint triage** (#181); a **signed, independently-verifiable benchmark** + `make bench` (#182); a **confidence-calibration report** + **coverage-guided oracle-gated (non-evasive) discovery** (#183); and the **`engage --learn` cross-run auto-loop** (persistent calibrator + Thompson bandit, non-circular). **Branch protection on `main` is live:** all **14** status checks are required on a pull request, the branch must be up to date with `main` before merging (`strict`), and force-pushes and branch deletion are blocked. Stated precisely, because the difference matters to an auditor: administrator enforcement is **off** (`enforce_admins: false`), so the repository owner retains an explicit admin override and *can* merge without green checks. The gate binds every ordinary contributor and every automated agent unconditionally; for the owner it is a deliberate, attributable act, not an impossibility. |
...
| 🆕 **2026-08 audit-hardening + UI-completeness + E1 wiring (merged & CI-green)** | An external-audit remediation sweep + a "controllable from the front" UI program, each PR verified → independently red-penned (where security-affecting) → CI → merged. **Audit A-series:** the egress **gateway hardened** (A7 #272 — client bind/auth, CONNECT + HTTP-port allowlist, Host-header binding, anti-smuggling, socket timeouts, and a scoped nftables backstop that no longer breaks the host/co-tenants); the **Rust WARDEN kernel** durability closed (A10 #285 — an `flock`'d sequence counter, an atomic temp+rename head write, fail-closed on an unwritable audit, a de-hardcoded interpreter path, and the 30 `cargo test`s wired into CI as their own required check); and the `request_smuggling` timing detection corrected from FACT to **UNCONFIRMED LEAD** (A12 #269) — **re_verified_closed in Wave-4.2**: the timing signal is retired and `request_smuggling` is re-promoted to **fact_capable** via a DIFFERENTIAL desync proof (a unique per-probe canary echoed in VIGIL's OWN second request on the conflict connection and absent from a well-formed control; `request_smuggling.differential_desync`, `smuggling_desync_oracle` + the `validate_action`-gated raw-socket re-drive), with shared-pool/HTTPS/HTTP-2 desyncs bounded as a residual LEAD. Verified-first: A7 and every O-series overclaim were confirmed already-honest and **not** re-touched. **UI-completeness (#273–#283):** a universal env-var config plane, `vigil services up`/`vigil doctor` (docker create-if-absent), a crash-proof `vigil up`, an **MCP Servers** and a **System & Services** screen, a **live soundness-benchmark** button (runs the same 11\|0\|0 the gate runs), Brain planner-projection + **OFFLINE** intel-recon launch, owner-signed agent-promotion controls, and read-only-screen exports — growing the UI to a **33-screen** NAV at the time. **E1-Slice3 (#286):** the IMDS/metadata credential-capture capability wired end-to-end **offline** (evidence branch + admission/verdict route + D2 certificate + attack-path/world-model projection; the D2 cert and the veracity firewall engage automatically because both are generic over any finding's retained `oracle_context`). **Deferred (honestly marked):** real-transport IMDS **live-fire** (credential-gated — no live FACT claimed yet); and LWW governance-record anti-replay (an `issued_at`-epoch fix, its own slice — a re-toggle-vs-replay design constraint ruled out the simpler signature-dedup). |
| 🆕 **2026-08 supply chain + cloud/K8s exploitation + sovereignty (merged & CI-green; full detail in [`docs/AS-BUILT.md`](docs/AS-BUILT.md) §2.3)** | **A14 supply-chain hardening (#294)** — digest-pinned base images, **two real hash-locked dependency locks** (one per environment, proven installable under `--require-hashes` in CI), an SBOM, and a **blocking CVE gate** (`trivy fs --severity HIGH,CRITICAL --exit-code 1` — raised from CRITICAL-only to HIGH+CRITICAL in **W3-9 (#432)** once the vendored HIGH backlog was cleared) whose **negative control** runs the blocking configuration against a known-CRITICAL fixture *and* a HIGH-only fixture and hard-fails if either comes out wrong — the gate's ability to fire, and to fire on a HIGH, is demonstrated, not asserted. Policy, thresholds and honest scope: [`docs/SUPPLY-CHAIN.md`](docs/SUPPLY-CHAIN.md). The first CVE it surfaced was **fixed, not ignored**: `cryptography` raised past **CVE-2026-69247 (HIGH)** in both environments (#295). **E-series achieved-effect cloud/K8s oracles (#286–#293)** — IMDS credential capture (E1), exposed-secret **validity** (E5), GCP service-account impersonation (E3), IAM **privilege escalation** as a strict-gain closure differential (E2), and two Kubernetes RBAC branches (E4). **E4's live-fire harness runs in a scheduled (nightly, non-blocking) CI job (#297; wired W11-2 #483)** — a throwaway single-node k3s cluster the job stands up and destroys, with the harness exiting non-zero on any deviation; what a *required* PR check proves is narrower and honestly so: the fail-closed-on-missing-tool preflight that guards every live-fire job (`tools/livefire/require_tools.py`, registered claim W11-2). The nightly real-cluster run itself is not a required gate (it was previously a manual run by hand). **E1/E2/E3/E5 are fixture-proven and credential-gated**, and **no live cloud FACT is claimed or exists** (runbook: [`docs/DEFERRED-INFRA.md`](docs/DEFERRED-INFRA.md) §E). **Sovereignty now governs LLM egress (#300)** — the tier was enforced at backend construction but the calls `vigil engage` actually made went around it; every model-egress site (engage think step, auto-patch coder, cockpit terminal router) now routes through the same `kernel.sovereignty` decision, fail-closed, with the tier carried into child processes and written into `sigil.env`. The default tier remains `PERMISSIVE` — the ladder binds, but you choose your rung. **CI (#299)** — `veracity` (the anti-hallucination firewall itself), `kernel`, `planner`, `sensors`, `eval`, `defender`, `memory` and `analysis` are now covered, and the flagship URL→report end-to-end test is repaired. **The Strix shell gate fails CLOSED (#296)**: a wiring failure raises `WardenGateUnavailable` and stops the run instead of continuing ungated. |
...
> **Every feature, and how it works:** the exhaustive per-feature catalog — **260+ features across 7 domains** (a pre-2026-08 lower bound; the audit-hardening, UI-completeness, and E1-wiring work above adds more), each with its
...
> decide), the **Terminal** UI screen, and the `vigil terminal` CLI verb — plus WARDEN-gating of the
> Strix `exec_command` shell (`VIGIL_WARDEN_STRIX_GATE`), which #157 landed as *gateable/opt-in* and which is
> now **ON BY DEFAULT and FAIL-CLOSED** (#178 made it the default; #296 made a wiring failure raise
> `WardenGateUnavailable` and stop the run — see the row above). The
...
Security posture (informational — off-by-default controls; does NOT affect the exit code):
  ...
  .. entitlement-anchor: DEFAULT-IN-TREE  — the entitlement (capability-enforcement) trust root uses the default in-tree (offense-writable) location; for a governed deployment anchor it out-of-band via CRUCIBLE_ENTITLEMENT_DIR on a read-only/HSM mount (docs/runbooks/anchor-trust-root-out-of-band.md)
  .. authority-root-anchor: DEFAULT-IN-TREE  — the remote-engage launch-gate authority root uses the default in-tree (offense-writable) location; for a governed deployment anchor it out-of-band via VIGIL_AUTHORITY_ROOT_DIR on a read-only/HSM mount (docs/runbooks/anchor-trust-root-out-of-band.md)
  ...
  .. witness:     SOLO  — no production witness roster — the shipped default is a solo self-witness (rollback DETECTION, not split-view prevention); provision >=2 DISTINCT witnesses with `python -m vigil_integration.witness_provision add`
  .. owner-key-backend: FILE  — owner key is a file-based Ed25519 key loaded into the process (the default). Set VIGIL_OWNER_KEY_BACKEND=pkcs11 to hold it in a hardware token instead.
```
...
For a real deployment the safe-by-default posture is the *wrong* default: you want the machine to **refuse to run** when a control it depends on is off. Set `VIGIL_POSTURE=production` (or `prod`) and a start path — `vigil up` or `vigil engage` — **refuses to start** unless **all eight** of these production preconditions hold:
...
- **`egress-supervisor`** is `ARMED` (**W10-8**). The seccomp connect/sendto/sendmsg supervisor (`tools/egress-guard`) is off-by-default and opt-in outside production; under `VIGIL_POSTURE=production` it is **forced on in `require` (fail-closed) mode** regardless of `VIGIL_EGRESS_GUARD`, so the gate **refuses to start** unless its binary is built (`make -C tools/egress-guard`) — otherwise every guarded spawn would be refused. **Its stated bound:** it refuses a *tool's own* non-loopback egress and a mis-built argv; it is **not** a containment boundary for hostile code (that boundary is `sandbox_exec` / bwrap `--unshare-all`), and it does **not** cover a 32-bit (i386) binary, `sendmmsg`, or `io_uring` submissions. It is therefore never sold as a complete egress control — see `docs/decisions/W10-8-egress-supervisor-production-gate.md` and the `egress_guard.py` module docstring for the full HONEST BOUNDs.
- **`witness`** is `DISTINCT-QUORUM` (**W8-3**). A witnessed anti-rollback checkpoint co-signed by **one** witness is rollback **DETECTION** via off-box retention, not split-view **PREVENTION** — and solo is the shipped default. Under `VIGIL_POSTURE=production` the gate **refuses to start** unless the transparency-log witness roster is a **strict majority of DISTINCT witnesses** (`2·threshold > n` over `n` distinct canonical keys — the exact rule already merged in the transparency log as `transparency.is_split_view_resistant`, **reused** here). Provision one end-to-end with no hand-editing: `python -m vigil_integration.witness_provision add` mints a new witness key and registers it with a recomputed strict-majority threshold; `register --pubkey <B64> --key-id <id>` adds an **out-of-band independent** witness (one an unrelated party runs via `python -m vigil_integration.witness_service serve` on their own box and advertises at `GET /pubkey`). **Negative control:** two witnesses that share a canonical key are refused as **NOT distinct** (any base64 encoding of one key collapses to one point), so a roster cannot fake a quorum from a single key. **Its stated bound:** distinctness of keys is checkable; **independence of the parties** custodying them is a deployment property code cannot verify — a strict-majority roster whose keys are all held by one operator is arithmetically resistant but provides no real independence. See `docs/decisions/W8-3-witness-provisioning-production-gate.md`.

The last one is **W10-7**. The cockpit's legacy embedded shared owner token always resolves to the owner — a deliberate fail-open so the operator physically at the host is never locked out. That is fine as a development convenience and **indefensible in a deployment someone else runs**, so it is a **named residual** that the production gate refuses to start around: set `SIGIL_LEGACY_OWNER_TOKEN=0` and provision per-user accounts (`sigil accounts create <user> <role>`, owner-signed offline — no HTTP token needed) that log in by proof-of-possession. Belt-and-suspenders: the sigil server **also refuses that shared token at runtime** whenever `VIGIL_POSTURE=production`, regardless of the toggle, so even a server started outside `vigil up` cannot honour it in production. (The claims registry — [W0-3] #398 — now exists in this tree (`docs/claims/registry.json`, guarded by `docs/tests/test_claims_registry.py`); the proving tests for this residual exist and run in required CI jobs today, and registering this specific residual as its own registry entry is a tracked follow-up.)

The gate is **additive and opt-in**: with `VIGIL_POSTURE` unset (or any non-production value) it is **inert** — behaviour is byte-identical to a fresh checkout, so nothing already deployed breaks. It is **fail-closed**: any precondition not in its required state — `UNKNOWN` included — refuses the start, with **one line per unmet precondition** naming the failing control and how to satisfy it. (The `egress-gate` control — the docker-gateway topology — is deliberately *not* one of the preconditions: a loopback engagement legitimately needs no docker gateway. That is distinct from `egress-supervisor`, the seccomp syscall guard above, which **is** a precondition.) When armed, `vigil doctor` doubles as the production preflight: it prints a **`PRODUCTION posture gate`** block and exits non-zero while any precondition is unmet. On a fully-misconfigured host that block reads:
...
```text
PRODUCTION posture gate (VIGIL_POSTURE=production) — REFUSES to start: 8 precondition(s) unmet (each blocks `vigil up` / `vigil engage`):
  ...
  !! egress-supervisor:  OFF  — the seccomp egress supervisor must be ARMED — build its binary (`make -C tools/egress-guard`) so production forces it on (VIGIL_EGRESS_GUARD=require, fail-closed). It supervises connect/sendto/sendmsg to refuse a tool's own non-loopback egress; it is NOT a containment boundary for hostile code and does NOT cover 32-bit binaries, sendmmsg or io_uring (see docs)
  !! witness:            SOLO  — the transparency-log witness set must be a STRICT MAJORITY of DISTINCT witnesses (W8-3) — a solo self-witness is rollback DETECTION, not split-view PREVENTION. Provision >=2 distinct witnesses with `python -m vigil_integration.witness_provision add` (mint one) or `register` an out-of-band witness key; witnesses that share a canonical key are refused as NOT distinct. Split-view prevention additionally needs those keys held by INDEPENDENT parties (uncheckable by code)
```
...
| | **Terminal** | A governed *local-only* shell for inspection, with an AI chatbot on top: the AI **proposes** a command, the **allowlist + WARDEN gate + your Run click** decide, and every run is a signed, redacted record. Local read-only binaries only — no network, no writes, no interpreter — by construction. (One of the UI's **34 screens** — see the canonical `knowledge/system-map/system-map.json`, gated to match `app.js`'s NAV.) |
| | **Live** | Real-time run view over the signed reasoning spine (SSE) — the FACT-vs-LEAD reasoning graph, approvals. |
| | **Strix Control** | The Strix agent's runtime control state (read-only): egress gateway + sandbox status, the pending WARDEN approval queue, proof health (the S9 degraded state), the **FACT / LEAD / CLEAN / INCONCLUSIVE** verdict separation, model locality + data residency, resource state, kill controls (host-pid stop + engagement kill-switch; container-reap is honestly marked *not yet wired*), and resume/recovery. Spawns nothing and mints no fact. |
...
- **Merged this cycle (PR #157):** the governed local Terminal (`execute_terminal`) + its natural-language AI chatbot + the Terminal UI screen + the `vigil terminal` CLI verb, and WARDEN-gating of the Strix shell (`VIGIL_WARDEN_STRIX_GATE`) — opt-in as #157 shipped it, **on by default since #178 and fail-closed since #296** (a wiring failure raises `WardenGateUnavailable` and stops the run; only the explicit env opt-out runs ungated). The session-omniscient **T2b** layer (session Q&A, cross-session fusion, ASK/DO modes, a signed replayable transcript in the dossier, teach-mode) is **roadmap** — see [`docs/VISION.md`](docs/VISION.md).
