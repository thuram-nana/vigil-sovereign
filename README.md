...
| | **Terminal** | A governed *local-only* shell for inspection, with an AI chatbot on top: the AI **proposes** a command, the **allowlist + WARDEN gate + your Run click** decide, and every run is a signed, redacted record. Local read-only binaries only — no network, no writes, no interpreter — by construction. (One of the UI's **32 screens** — see the canonical `knowledge/system-map/system-map.json`, gated to match `app.js`'s NAV.) |
