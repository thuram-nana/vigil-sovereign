...
if [ ! -f "$SIGIL_HOME/sigil.env" ]; then
  {
    ...
    echo "#"
    echo "# --- Sovereignty: what may leave this machine -------------------------------"
    echo "# CRUCIBLE_SOVEREIGNTY_TIER governs every model/LLM call the engine makes,"
    echo "# including the 'vigil engage' think step. Left UNSET it is PERMISSIVE (the"
    echo "# development default: any backend, including the direct Anthropic API)."
    echo "# Uncomment ONE to constrain egress. A disallowed backend is refused before the"
    echo "# SDK is even imported, so nothing leaves the host."
    echo "#   CRUCIBLE_SOVEREIGNTY_TIER=AIR_GAPPED       # local backends only (ollama/vLLM/llama-cpp/TGI)"
    echo "#   CRUCIBLE_SOVEREIGNTY_TIER=SOVEREIGN_CLOUD  # + jurisdictional cloud (Bedrock/Vertex/Mistral)"
    echo "#   CRUCIBLE_SOVEREIGNTY_TIER=TRUSTED_CLOUD    # + Anthropic ZDR (also set CRUCIBLE_ANTHROPIC_ZDR=1)"
    echo "#   CRUCIBLE_SOVEREIGNTY_TIER=PERMISSIVE       # anything (the explicit form of the default)"
    echo "# CRUCIBLE_SOVEREIGNTY_SEALED=1 latches the tier for the process lifetime, so a"
    echo "# later environment change cannot relax it mid-run."
    echo "# ---------------------------------------------------------------------------"
