...
- **Model egress defaults to permissive.** With no sovereignty tier set, the resolved tier
  ...
  Settings screen reaches an offense process only when `vigil up` launched it. The sovereign runtime
  env is resolved at bring-up and **re-resolved at each offense-plane restart** (W10-2 #474), so a tier
  changed in Settings while the UI is running takes effect on the next offense-plane restart or a fresh
  `vigil up`, but not on a run already in flight. Started any other way, the process falls back to
  `PERMISSIVE` **silently — fail-open**
  (`engine/crucible/framework/v2/kernel/sovereignty.py:183-195`; scope of each mechanism in
  `ACCEPTABLE-USE.md`, Sovereignty tier). Note the tier's stated limits, including that it does not govern the
...
**Legal compliance.** Including export control and sanctions, and
