...
# operator explicitly widens it. Both are Tier-1, entitlement-free, no-egress, and — critically —
# take NO local-file/path argument (so an MCP caller cannot drive a file read):
#   * reverify_finding — re-execute a finding's oracle certificate OFFLINE (read-only, no host).
#   * declared_service — normalise operator-declared host services (host-scoped, no side effect).
# A property-only filter is deliberately NOT the default: several other Tier-1 sensors (a pcap or a
# results-file importer) accept a local path, and exposing those would let a remote caller read
# arbitrary local files. An ALLOWLIST is fail-closed — a new tool is never exposed until named.
...
def default_exposed_registry() -> ToolRegistry:
    ...
    Tier-1, entitlement-free, no-egress producers (``reverify_finding``, ``declared_service``) are
    reachable over MCP. Even those are still gated by ``invoke_tool`` on every call."""
