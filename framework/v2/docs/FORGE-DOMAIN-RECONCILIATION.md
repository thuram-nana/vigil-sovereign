...
| **7** | **Identity posture & anomaly** | **BUILT (slices 1–2)** | `IDENTITY_POSTURE` oracle (`verify/oracles.py::identity_posture_oracle`) + seam/ingest (`verify/identity_posture.py`) + sensor (`sensors/identity.py::IdentitySensor`, Tier-1, offline IdP export) + fusion (`engage_fusion.py::_reverify_identity`, opt-in). Four rules on the ONE kind: `privileged_without_mfa`, `stale_credential` (slice 1); `wildcard_grant` (admin_all / universal grant only — a partial wildcard refuses), `dormant_privileged` (slice 2). **anomaly/behavioral detection and cloud-resource IAM remain OUT of scope** (`POLICY_PATH`/`CLOUD_POSTURE` own IAM). Each slice cleared the standing dual gate (RED-PEN + independent adversarial-sweep); ledger in `V2-LIMITATIONS.md` §29. Only the **anomaly** half is unbuilt. |
...
Domain 7 (identity posture) is now BUILT at slices 1–2 (all four posture predicates; see the table above);
only the **anomaly** half remains a follow-up.
