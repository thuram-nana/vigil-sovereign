...
| **7** | **Identity posture & anomaly** | **BUILT (slice 1)** | `IDENTITY_POSTURE` oracle (`verify/oracles.py::identity_posture_oracle`) + seam/ingest (`verify/identity_posture.py`) + sensor (`sensors/identity.py::IdentitySensor`, Tier-1, offline IdP export) + fusion (`engage_fusion.py::_reverify_identity`, opt-in). Slice-1 predicates: `privileged_without_mfa`, `stale_credential`. `wildcard_grant`/`dormant_privileged` DEFERRED to a follow-up charter; **anomaly/behavioral detection and cloud-resource IAM are OUT of scope** (`POLICY_PATH`/`CLOUD_POSTURE` own IAM). Cleared the standing dual gate (RED-PEN + independent adversarial-sweep); ledger in `V2-LIMITATIONS.md` §29. |
...
Domain 7 (identity posture) is now BUILT at slice 1 (see the table above); its `wildcard_grant` /
`dormant_privileged` predicates and the anomaly half remain a follow-up charter.
