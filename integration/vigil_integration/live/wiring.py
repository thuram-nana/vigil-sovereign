...
def _redrive_branch_for(bug_class: str) -> "Optional[str]":
    ...
    mapping = {
        ...
        # Wave 3.3 — CSRF achieved re-drive (gated-workflow): a state-changing request VIGIL issued
        # cross-origin carrying ONLY the ambient session cookies of its own authenticated session (the
        # write through the 0.3 per-action approval), confirmed by the FACT-capable ACHIEVED_STATE branch
        # (verify/oracles.py:csrf_achieved_oracle) ONLY when a VIGIL-chosen unique marker reached the
        # authoritative post-state WITH the ambient cookie but is ABSENT from the no-cookie control. A
        # non-fire / no channel / a SameSite-protected cookie keeps the claim a LEAD (the weaker `csrf`
