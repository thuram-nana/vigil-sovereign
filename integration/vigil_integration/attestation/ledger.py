...
def verify_ledger(
    ...
    *,
    ...
    back-dated entry is caught). Ordered by ``seq`` (with ``prev_hash`` linkage), so verification is
    independent of the input list's order.

    WHEN / back-dating (W0-14 #409, RED-PEN reconcile): the anti-back-dating authority is the forward-only
    ``monotonic`` anchor, NOT the wall-clock ``at``. ``at`` is untrusted signed DATA — a settable clock
    that NTP can legitimately step BACK between two real records — so verify does NOT gate on its ordering:
    doing so would false-positive a benign clock step-back as "tampered" (and be unsound across timezones).
    Real back-dating is a rollback of ``monotonic``, which the non-decreasing check catches; ``at`` is
    advisory replay data (:func:`ledger_when`) only.
    ...
    for rec in ordered:
        ...
        # W0-14 #409 (RED-PEN reconcile) — the anti-back-dating authority is the MONOTONIC ANCHOR above,
        # NOT the wall-clock ``at``. ``at`` is untrusted signed DATA (a settable clock: NTP can step it
        # BACK between two legitimate records), so verify does NOT gate on it — an ``at``-ordering check
        # would raise a false "tampered" verdict on a benign clock step-back, contradicting the module's
        # own wall-clock-is-untrusted premise, and would in any case be unsound across timezones (a
        # lexicographic compare is chronological only within one fixed offset). Real back-dating is a
        # rollback of the forward-only ``monotonic`` counter, which the check above catches; ``at`` stays
        # advisory replay data (``ledger_when``), never a verify gate or an ordering key.
