...
def test_dependencies_report_is_structured_and_every_missing_item_is_actionable():
    ...
    for d in deps:
        ...
        # and a PRESENT one carries none (so the UI/operator never sees a red row with no remedy).
        if d["present"]:
    ...
    assert any(d["kind"] == "offense-tool" for d in deps)             # real-scan tools
    assert any(d["kind"] == "image" for d in deps)                    # engine images
...
def test_install_hints_are_non_empty_shell_commands():
    for tool, how in dmod._OFFENSE_TOOLS.items():
        assert how.strip() and ("apt" in how or "install" in how), f"{tool} hint is not a command: {how!r}"
...
def test_render_surfaces_install_commands_and_external_reachability():
    report = {
        ...
        "dependencies": [{"name": "nuclei", "kind": "offense-tool", "present": False, "required": False,
                          "detail": "real-scan tool", "how_to_install": "sudo apt install -y nuclei"}],
        "docker_services": {"qdrant": {"state": "absent", "purpose": "vector memory", "reachable": True}},
    }
    txt = dmod.render(report)
    assert "sudo apt install -y nuclei" in txt
    assert "serving on the port (external container)" in txt
