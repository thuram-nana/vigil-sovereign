...
def test_posture_probe_that_raises_yields_unknown_not_crash(monkeypatch, tmp_path):
    ...
    assert len(posture) == 9                                  # the other eight still produced
...
def test_doctor_reads_posture_without_importing_sigil_or_framework(tmp_path):
    ...
    assert res["controls"] == ["egress-gate", "vault", "key-sealing", "sovereignty", "entitlement", "backups", "charter", "egress-supervisor", "witness"]
