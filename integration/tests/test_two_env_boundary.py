...
import vigil_integration.live.authorization_broker  # Phase 1: the target-authorization seam (both planes)
assert "framework" not in _m, "authorization_broker must not import framework at module scope"
assert "strix" not in _m, "authorization_broker must not import strix at module scope"
...
import vigil_integration.live.imds_verify  # E1-Slice3: the IMDS credential-capture admission/cert producer
assert "framework" not in _m, "imds_verify must not import framework at module scope"
assert "strix" not in _m, "imds_verify must not import strix at module scope"
...
import vigil_integration.live.iam_escalation_verify  # E2: the IAM privilege-escalation admission/cert producer
assert "framework" not in _m, "iam_escalation_verify must not import framework at module scope"
assert "strix" not in _m, "iam_escalation_verify must not import strix at module scope"
...
import vigil_integration.live.gcp_impersonation_verify  # E3: the GCP SA-impersonation admission/cert producer
assert "framework" not in _m, "gcp_impersonation_verify must not import framework at module scope"
assert "strix" not in _m, "gcp_impersonation_verify must not import strix at module scope"
import vigil_integration.live.k8s_rbac_grant_verify  # E4 TIER-2: the RBAC verb-grant admission/cert producer
assert "framework" not in _m, "k8s_rbac_grant_verify must not import framework at module scope"
assert "strix" not in _m, "k8s_rbac_grant_verify must not import strix at module scope"
