...
# Wave 4.5 MFA BYPASS achieved-state oracle (plan §4.5): SAME frozen-fallback discipline — the NEW OracleKind
# (its OWN dedicated kind so ACHIEVED_STATE's oracle_version is untouched) is kept OUT of _ALL_ORACLES,
# reachable ONLY via its `mfa_bypass` BUG_CLASS_ORACLES row keyed on a `mfa_bypass` ctx field no benchmark/scan
# finding carries. It proves the ACHIEVED post-MFA state (CWE-287/CWE-308) under a fail-closed operator
# attestation + the same-shape private-read differential re-derived from raw bytes, not a posture weakness.
_W45_KINDS = {OracleKind.MFA_BYPASS}
_W45_CLASSES = {"mfa_bypass"}
# Wave 4.3 PASSWORD-RESET / ACCOUNT-RECOVERY TOKEN INVARIANT oracle (plan §4.3): SAME frozen-fallback
# discipline — the NEW OracleKind (its OWN dedicated kind so ACHIEVED_STATE's oracle_version is untouched) is
# kept OUT of _ALL_ORACLES, reachable ONLY via its `password_reset_reuse` / `password_reset_collision`
# BUG_CLASS_ORACLES rows keyed on a `password_reset_invariant` ctx field no benchmark/scan finding carries. It
# proves the invariant-free reset sub-properties (token reuse/non-expiry by a private-read reduction; a
# deterministic predictable-counter collision) — never a posture weakness. The CROSS-USER reset class reuses
# ACHIEVED_STATE (so it is NOT a new-kind class and is NOT excluded here); HOST-POISONING folds onto
