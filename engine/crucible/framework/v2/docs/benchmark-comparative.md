...
vulnerable web app with a known ground truth of eleven planted bugs
(reflected XSS, boolean-blind SQLi, error-based SQLi, open redirect,
path traversal, SSTI, host-header injection, CORS-with-credentials, and
three exposures: `.git/config`, `.env`, and Spring `/actuator/env`) plus
five SAFE endpoints (`/profile`, `/api/health`, `/download`, `/greeting`,
`/support`) that must never be flagged. Because the
...
**CRUCIBLE result:** precision 1.000 (MEETS target), recall 1.000, f1 1.000 (tp=11, fp=0, fn=0).
...
Cost of the same runs — wall-clock, active requests issued, and
best-effort peak RSS. A `-` means the tool does not report that
number (an incumbent CRUCIBLE shells out to does not expose its
internal request count); it is left blank rather than faked to 0.
