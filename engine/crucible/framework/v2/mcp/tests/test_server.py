...
def test_tools_list_exposes_only_the_safe_allowlist() -> None:
    ...
    # the active/offensive sensors, egress importers, AND local-path readers are NOT advertised
    assert not ({"nmap", "nuclei_web", "nuclei_import", "tshark_flow", "burp_web", "sbom_vuln"} & names)
...
def test_local_path_and_active_tools_stay_unexposed_and_refused() -> None:
    # sbom_vuln reads a caller-influenced local file PATH → DELIBERATELY excluded from the default
    # allowlist (the MCP surface must not become a local-fs oracle), and an ACTIVE sensor present in
    # the same registry is NOT exposed and a call to it is REFUSED by the expose policy (before the
    # invoker) — the property re-check (Tier-2 / active_recon) keeps it unreachable.
    server = MCPServer(slug="alpha")
    names = {t["name"] for t in server.handle(_rpc("tools/list"))["result"]["tools"]}
    assert "sbom_vuln" not in names and "nmap" not in names
