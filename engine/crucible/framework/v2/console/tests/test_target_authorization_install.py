...
def test_install_then_verified_authority_gate_passes(env):
    ...
    # DECOUPLED STORE (Phase 0.1): the install persists the authority trust root to the DEDICATED authority-root
    # store, NOT the entitlement store (writing there would trip entitlement capability enforcement).
    assert paths.authority_root_path().is_file()
...
def test_gate_rejects_a_verified_but_out_of_window_authority(env):
    ...
    # DECOUPLED STORE (Phase 0.1): _has_verified_authority reads the DEDICATED authority-root store, so provision
    # the trust root there (write_authority_root), not the entitlement store.
...
def test_malformed_existing_trust_root_is_not_overwritten(env):
    ...
    from framework.v2.common import paths
    # DECOUPLED STORE (Phase 0.1): the install reads the DEDICATED authority-root store — plant the malformed root
    # THERE (not the entitlement store) so the present-but-unreadable conflict is exercised.
    tp = paths.authority_root_path()
...
def test_conflicting_existing_trust_root_is_not_replaced(env):
    # DECOUPLED STORE (Phase 0.1): the install reads the DEDICATED authority-root store — provision the pre-existing
    # (different) trust root THERE (write_authority_root), not the entitlement store.
