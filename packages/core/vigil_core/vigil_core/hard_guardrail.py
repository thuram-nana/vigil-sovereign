"""
hard_guardrail — a deterministic scope floor for categorically-never targets (VIGIL-FUSION F1).
...
evaluated **before the charter or any gate is consulted** and with **no LLM, no network dependency**.
It is defense-in-depth, not a replacement for the charter: the charter still authorizes what IS in
scope; this guard removes an entire class from consideration so that a prompt-injection or an operator
typo can never redirect an autonomous agent at, say, a government or military host.

Two halves, kept deliberately separate:

  * The MATCHER — :func:`is_hard_blocked` / :func:`assert_not_hard_blocked` / :func:`candidate_hosts` /
    :func:`normalize_domain`. Pure, deterministic, **env-free**: no LLM, no network, no settings. These
    are byte-identical to the F1 original and are the categorical predicate.
  * The TOGGLE READER — :func:`protected_guard_enabled`. Reads a single owner-controlled env var
    (``VIGIL_ALLOW_PROTECTED_DOMAINS``) to decide whether callers should ENFORCE the matcher. Fail-safe:
    unset / empty / junk ⇒ enabled (protected). The only OFF state is an explicit affirmative that the
    owner wrote via the signed config plane and that ``export_runtime_env`` delivered to this plane.

VIGIL note: this is a pure predicate that can only DENY — it never authorizes anything and is not an
authority. It runs first; if it does not block, the normal charter + conjunctive gate still decide. The
toggle governs a defense-in-depth pre-filter ONLY; the signed charter scope and the never-liftable egress
floor remain in force regardless of the toggle.

Placement (Slice 0 / FATAL-2): this module lives in the neutral shared ``vigil_core`` (pure stdlib,
namespace-pure — imports NO ``framework.*``/``strix.*``/``sigil.*``) so BOTH the offense engine
(``framework.v2``) and the integration plane (``vigil_integration``) import the SAME floor without a
reversed ``framework → vigil_integration`` import edge. ``vigil_integration.safety.hard_guardrail``
re-exports it (back-compat for every existing importer), mirroring the ``vigil_core.gate`` precedent.

Pure stdlib. Import-clean (no ``framework.*``/``strix.*``/``sigil.*``).
"""
...
# The owner toggle. Polarity is deliberately ALLOW (the deviation), not the protection, so an unset/
# empty/cleared value is fail-SAFE (protected). See :func:`protected_guard_enabled`.
_ALLOW_ENV = "VIGIL_ALLOW_PROTECTED_DOMAINS"
...
def protected_guard_enabled() -> bool:
    """True (guard ACTIVE / protected) unless the owner has EXPLICITLY turned it off. Fail-safe: unset,
    empty, whitespace, or any non-affirmative value → True. The only OFF state is an explicit affirmative
    in ``VIGIL_ALLOW_PROTECTED_DOMAINS`` (``1``/``true``/``yes``/``on``, case/space-insensitive), written
    owner-signed via ``settings.set_config`` and delivered to this (offense) plane by ``export_runtime_env``.
    This governs a DEFENSE-IN-DEPTH pre-filter only; the signed authority scope + the egress floor remain
    in force regardless of this flag."""
    return os.environ.get(_ALLOW_ENV, "").strip().lower() not in ("1", "true", "yes", "on")
