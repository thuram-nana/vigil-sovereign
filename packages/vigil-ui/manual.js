...
window.VIGIL_MANUAL = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { h: "The gated ladder" },
      { p: "First a model proposes the fix as a minimal diff — that step is not gated because it writes nothing, but it is load-bearing: with no model reachable nothing is proposed and the run ends with no patch at all. Then the ladder runs: clone the repo into a disposable copy, apply the proposed edits there, apply-and-check the patch, and open a pull request. The WARDEN gate puts every one of those four steps at A2 — above the offense auto-ceiling — so none of them ever runs on its own. The first three QUEUE for your approval; the pull-request step is refused outright unless you have separately switched it on and had it multi-signed, so it never happens from the screen at all." },
      { p: "You give that approval ONCE, UP FRONT: the Apply button on the Fixes screen, or `--apply-edits --approve` on the command line (`--apply-edits` alone is not enough — without `--approve` the ladder cannot even clone). It pre-approves every file in the proposal; there is NO per-file prompt on either path, and without that opt-in every proposed file times out and is REJECTED. Each file is still gated, path-checked and deadline-checked individually, and only explicit, path-validated files are ever staged — never a bulk change. The edits land in a disposable clone, so your own tree is never touched." },
      ...
      { note: "Live auto-application is the `vigil patch` verb — run it from the command line, or click Apply on the Fixes screen, which shells that same gated verb for you (never `--open-pr`). Either way it runs only over a PROVEN finding, never a raw file you hand it. The default is a dry run that touches nothing but a throwaway clone; opening a real PR is off by default and needs the signing key set up below, plus a separate multi-signer (m-of-n) authorization — it is never opened from the screen." },
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      { p: "Add the secrets the system uses here — your AI provider API key; a GitHub token (for live auto-patch) that lets the fix engine push a branch and open a gated pull request; and the auto-patch signing key that AUTHORIZES a PR (generate it with `vigil provision-destruction` — see “Arming live auto-patch”). Each is sealed into your keyring or TPM vault WHEN AVAILABLE; otherwise it is stored 0600 in ~/.sigil/sigil.env, which is not sealed (the Seal toast tells you which happened). Either way it is shown to you only as a redacted fingerprint afterwards, and is NEVER written to the audit log or sent to your browser again. All three are optional — you can run keyless (deterministic checks only, no AI reasoning), and you only need the GitHub token and signing key if you open PRs." },
      { p: "Choose which AI model VIGIL thinks with — the model you pick here is the one the engine actually reasons with (it flows to both engagements and fix proposals). Every secret is sealed when a keyring or TPM vault is available (otherwise stored 0600 in ~/.sigil/sigil.env, which is not sealed) and delivered to the engine on your machine only." },
    ],
  },
  {
