...
## Corpus Check
- 391 files · ~886,657 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5035 nodes · 18416 edges · 102 communities detected
- Extraction: 40% EXTRACTED · 60% INFERRED · 0% AMBIGUOUS · INFERRED: 11097 edges (avg confidence: 0.62)
...
1. `HttpRequest` - 265 edges
2. `WorldModel` - 246 edges
3. `NodeKind` - 212 edges
4. `Edge` - 212 edges
5. `CrucibleError` - 188 edges
6. `worldmodel — the persistent, typed attack-graph substrate.  Every other reasonin` - 187 edges
7. `EdgeKind` - 168 edges
8. `InsertionKind` - 166 edges
9. `FindingContext` - 165 edges
10. `Node` - 156 edges
...
- `The lowercased technology names present (e.g. ``{"nginx", "php"}``).` --uses--> `CrucibleError`  [INFERRED]
  /home/kali/Pictures/PENTEST-main/framework/v2/scanner/fingerprint.py → /home/kali/Pictures/PENTEST-main/framework/v2/common/errors.py
- `The categories present (e.g. ``{"server", "language", "cms"}``).` --uses--> `CrucibleError`  [INFERRED]
  /home/kali/Pictures/PENTEST-main/framework/v2/scanner/fingerprint.py → /home/kali/Pictures/PENTEST-main/framework/v2/common/errors.py
...
Cohesion: 0.01
Nodes (522): FindingContext, from_timing_samples(), Typed, replayable bundle of the observations one finding is judged on.      A fi, detect_outliers(), _median(), intruder.analysis — anomaly detection over an attack's result population.  Burp, Indices of anomalous rows.      A row is flagged if ANY of: its status code is a, _Row (+514 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (343): from_http_responses(), Agent, detect(), api_detection — identify API patterns (REST / GraphQL / SOAP / RPC)., detect(), auth_detection — identify the authentication scheme(s)., Agent, agents.base — common Agent interface.  Every specialist agent subclasses `Agent` (+335 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (398): AttackerState, worldmodel.attacker — the attacker's own state as first-class, persistent facts., Record that the attacker has reached ``service_id`` (a service /         endpoin, Node ids the attacker currently controls (deterministic order)., Credential/session/token ids the attacker currently holds., Service/endpoint/segment ids the attacker has reached., A thin, typed view over a :class:`WorldModel` for recording and querying     wha, Idempotently add the attacker principal node. Returns its id. (+390 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (216): from_predicate(), from_state(), Fold another context's populated inputs into this one, so a single         findi, main(), request_one(), PatternAnalyzer, _r(), analysis.analyzers.builtin — the offline pattern analyzer.  A real, dependency-f (+208 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (292): _coerce_text(), from_boolean_probes(), from_oob(), from_process_output(), from_side_effect(), _hit_to_dict(), verify.adapter — translate already-collected observations into oracle inputs.  `, Keep mappings/lists as-is (JSON-safe, and the side-effect oracle searches     th (+284 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (250): build_engagement_allowlist(), EgressAllowlist, agents.egress_guard — runtime egress allowlist for sovereign-mode httpx.  Why th, Construct an allowlist for one engagement.      Reads the charter scope via `eth, httpx transport that refuses requests to hosts outside the     allowlist. Wraps, Hosts permitted to receive HTTP requests from this process.      Three categorie, SovereignHttpxTransport, EngagementRefused (+242 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (196): AdapterError, _burp_issue_list(), BurpAdapter, NucleiAdapter, parse_burp(), parse_nuclei(), parse_sqlmap(), parse_zap() (+188 more)

### Community 7 - "Community 7"
Cohesion: 0.03
Nodes (174): AnthropicBackend, AnthropicBackend — live LLM via the Anthropic Messages API.  Activates when:   -, Live Anthropic backend. The ZDR variant differs only in `name`     (which the so, BaseModel, BedrockBackend, BedrockBackend — Claude on AWS Bedrock with regional restriction.  Sovereign-clo, _region_allowlist(), build_system_prompt() (+166 more)

### Community 8 - "Community 8"
Cohesion: 0.04
Nodes (204): binding_satisfied(), current_host_identifiers(), The set of identifiers the running host can present. Compared,     case-sensitiv, Return (ok, reason). A 'none' binding is always satisfied. A     'host_attestati, _read_machine_ids(), _canonical_json(), entitlement_signing_bytes(), improve.canonical — deterministic signing bytes for a proposal.  Governance appr (+196 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (179): body(), cookie(), evaluate(), _evaluate_signature(), hdr(), _matches(), path(), intake.fingerprint._common — shared signature engine.  Every detector is a list (+171 more)

### Community 10 - "Community 10"
Cohesion: 0.03
Nodes (179): BenchmarkReport, _bin_edges(), brier_score(), Calibrator, _clamp(), fit(), measure_ece(), pav() (+171 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (158): authority_signing_bytes(), The exact bytes a governance approver signs to approve a merge., authority_from_charter(), authority_from_scope(), authority.charter — derive an engagement authority from the charter.  The charte, Build an authority from an explicit scope list. Fail closed on an     empty scop, Build an authority from the charter's in-scope host table. Raises     OutOfScope, _authorize() (+150 more)

### Community 12 - "Community 12"
Cohesion: 0.02
Nodes (134): What a crawl found: the fuzzable requests (ready for ``AuditEngine.audit``),, What a crawl found: the fuzzable requests (ready for ``AuditEngine.audit``),, Breadth-first, scope-bounded, cycle-safe crawler. ``send(HttpRequest) ->     {st, Breadth-first, scope-bounded, cycle-safe crawler. ``send(HttpRequest) ->     {st, Canonical location key: identical param *names* (not values) collapse, so     ``, Canonical location key: identical param *names* (not values) collapse, so     ``, CrucibleError, analyze_html() (+126 more)
...
Cohesion: 0.02
Nodes (108): loopback_send(), Issue one request against a loopback target and return     ``{status, body, head, _check_bound(), bit_flipper(), brute_force(), case_variations(), char_blocks(), dates() (+100 more)
...
Cohesion: 0.05
Nodes (62): diff_summary(), main(), make_request(), Print per-class summary., Compare valid vs invalid stats., Make one login attempt with the wrong password., summarize(), arm_key() (+54 more)
...
- **Why does `HttpRequest` connect `Community 0` to `Community 3`, `Community 7`, `Community 10`, `Community 11`, `Community 12`, `Community 14`, `Community 18`, `Community 21`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `CrucibleError` connect `Community 2` to `Community 0`, `Community 1`, `Community 3`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 18`, `Community 20`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Are the 259 inferred relationships involving `HttpRequest` (e.g. with `JwtNoneCheck` and `scanner.jwt — JSON Web Token analysis and attacks.  JWTs are their own attack su`) actually correct?**
  _`HttpRequest` has 259 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 201 inferred relationships involving `str` (e.g. with `main()` and `.__init__()`) actually correct?**
  _`str` has 201 INFERRED edges - model-reasoned connections that need verification._
- **Are the 209 inferred relationships involving `NodeKind` (e.g. with `ProposedHop` and `ChainResult`) actually correct?**
  _`NodeKind` has 209 INFERRED edges - model-reasoned connections that need verification._