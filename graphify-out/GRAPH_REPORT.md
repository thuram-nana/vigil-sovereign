...
## Corpus Check
- 483 files · ~488,180 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6855 nodes · 27224 edges · 161 communities detected
- Extraction: 36% EXTRACTED · 64% INFERRED · 0% AMBIGUOUS · INFERRED: 17477 edges (avg confidence: 0.61)
...
3. `WorldModel` - 363 edges
4. `NodeKind` - 354 edges
5. `CrucibleError` - 288 edges
6. `EdgeKind` - 271 edges
7. `Edge` - 267 edges
8. `WebScanCampaign` - 265 edges
...
Cohesion: 0.01
Nodes (598): FindingContext, from_predicate(), from_state(), from_timing_samples(), Typed, replayable bundle of the observations one finding is judged on.      A fi, detect_outliers(), _median(), intruder.analysis — anomaly detection over an attack's result population.  Burp (+590 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (454): from_http_responses(), Fold another context's populated inputs into this one, so a single         findi, Agent, detect(), api_detection — identify API patterns (REST / GraphQL / SOAP / RPC)., main(), request_one(), detect() (+446 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (444): authority_full(), _authority_state(), benchmark_data(), coverage_data(), engagement_detail(), _engagement_row(), intel_data(), kernel_data() (+436 more)
...
Cohesion: 0.02
Nodes (405): AdapterError, _burp_issue_list(), BurpAdapter, AdapterError, _nikto_class(), _nikto_hosts(), _nikto_location(), NiktoAdapter (+397 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (368): binding_satisfied(), current_host_identifiers(), The set of identifiers the running host can present. Compared,     case-sensitiv, Return (ok, reason). A 'none' binding is always satisfied. A     'host_attestati, _read_machine_ids(), authority_signing_bytes(), _canonical_json(), entitlement_signing_bytes() (+360 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (337): _coerce_text(), from_boolean_probes(), from_dom_execution(), from_error_signature(), from_evaluation(), from_oob(), from_process_output(), from_side_effect() (+329 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (287): AnthropicBackend, AnthropicBackend — live LLM via the Anthropic Messages API.  Activates when:   -, Live Anthropic backend. The ZDR variant differs only in `name`     (which the so, BaseModel, BedrockBackend, BedrockBackend — Claude on AWS Bedrock with regional restriction.  Sovereign-clo, _region_allowlist(), build_system_prompt() (+279 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (238): build_engagement_allowlist(), EgressAllowlist, agents.egress_guard — runtime egress allowlist for sovereign-mode httpx.  Why th, Construct an allowlist for one engagement.      Reads the charter scope via `eth, Construct an allowlist for one engagement.      Reads the charter scope via `eth, httpx transport that refuses requests to hosts outside the     allowlist. Wraps, httpx transport that refuses requests to hosts outside the     allowlist. Wraps, httpx transport that refuses requests to hosts outside the     allowlist. Wraps (+230 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (197): Trip the kill-switch for an engagement — the emergency stop. Idempotent (the, trip_killswitch(), PatternAnalyzer, _r(), analysis.analyzers.builtin — the offline pattern analyzer.  A real, dependency-f, Offline pattern-based static analyzer. Always available., _Rule, _as_dict() (+189 more)
...
Cohesion: 0.03
Nodes (188): BenchmarkReport, BlackboardEventRow, Materialised view of a row, with payload already deserialised., _bin_edges(), brier_score(), Calibrator, _clamp(), fit() (+180 more)

### Community 12 - "Community 12"
Cohesion: 0.02
Nodes (163): evaluate_predicate(), load_library(), select_entries(), _check(), _confirm(), _server(), test_sprt_check_confirms_boolean_sqli(), test_sprt_check_refutes_non_injectable_target() (+155 more)

### Community 13 - "Community 13"
Cohesion: 0.02
Nodes (131): console_dir(), launch_scan(), console.actions — the SAFE operator actions (the only mutations the console make, Spawn a loopback `scan` subprocess that streams progress + saves its report., Re-run the retained oracle certificates in a saved run's report — a pure,     of, reverify_run(), run_dir(), _write_meta() (+123 more)

### Community 14 - "Community 14"
Cohesion: 0.02
Nodes (104): _no_send(), main(), verify.collaborator_cli — `python3 -m framework.v2 collaborator serve`.  Runs th, port(), verify.collaborator — a self-hostable out-of-band interaction relay.  `verify.oo, A context-managed OOB relay the operator hosts on an allowlisted host.      ``ho, Run the relay in the foreground until interrupted (the CLI path)., Mint a fresh correlation token and its callback URL on the relay. (+96 more)

### Community 15 - "Community 15"
Cohesion: 0.04
Nodes (102): BlackboardError, agents.blackboard — append-only event log shared by every agent.  Per FORGE PROT, Insert a new event. Validates payload against the kind's schema., Convenience: post a new event with supersedes_id=old_id,         copying engagem, Anything wrong with a blackboard operation (validation, IO)., _regress(), builtin_corpus(), eval.corpus — load a benchmark corpus from disk.  Two accepted layouts:    1. Si (+94 more)
...
Cohesion: 0.04
Nodes (77): diff_summary(), main(), make_request(), Print per-class summary., Compare valid vs invalid stats., Make one login attempt with the wrong password., summarize(), confirm_dom_xss() (+69 more)
...
Cohesion: 0.05
Nodes (86): _emit(), _horizon(), _load_snapshot(), _now(), _review(), _timeline(), ingest_horizon(), load_horizon_feed() (+78 more)

### Community 20 - "Community 20"
Cohesion: 0.06
Nodes (82): _annotate(), _descriptor(), _rules(), _ruleset(), _score(), detection_gaps(), DetectionGap, defender.gap_report — "what would catch me": detection gaps + candidate rules. (+74 more)
...
Cohesion: 0.08
Nodes (29): authenticated_send(), AuthSession, _body(), CookieJar, LoginSequence, scanner.session — authenticated scanning (cookie jar + login + re-auth).  Most o, The authenticated ``send``: ensures a session, carries cookies, and         re-a, Run the login sequence; capture the session cookie. Success = the         login (+21 more)
...
- **Thin community `Community 143`** (1 nodes): `The exact mapping `OracleVerifier.confirm` consumes. Only keys whose         inp`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 144`** (1 nodes): `Side-effect reflection: place a unique canary (wrapped by `payload_template`)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 148`** (1 nodes): `A list of out-of-band interactions (whatever `OOBReceiver.poll`         returned`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 149`** (1 nodes): `An expected (attacker-predicted) vs. observed state pair for the         achieve`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 152`** (1 nodes): `Fold another context's populated inputs into this one, so a single         findi`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 153`** (1 nodes): `The fired signals; the subset that carried the confirmation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 158`** (1 nodes): `The exact mapping `OracleVerifier.confirm` consumes. Only keys whose         inp`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 159`** (1 nodes): `Mint a fresh correlation token and return (token, callback_url).          The UR`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 160`** (1 nodes): `Return all hits recorded against `token` so far (possibly empty).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `worldmodel — the persistent, typed attack-graph substrate.  Every other reasonin` connect `Community 2` to `Community 0`, `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 14`, `Community 15`, `Community 16`, `Community 17`, `Community 18`, `Community 19`, `Community 20`, `Community 21`, `Community 22`, `Community 24`, `Community 26`?**
  _High betweenness centrality (0.170) - this node is a cross-community bridge._
- **Why does `CrucibleError` connect `Community 0` to `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 7`, `Community 8`, `Community 9`, `Community 10`, `Community 11`, `Community 13`, `Community 15`, `Community 16`, `Community 17`, `Community 24`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `HttpRequest` connect `Community 0` to `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 7`, `Community 9`, `Community 10`, `Community 11`, `Community 12`, `Community 16`, `Community 22`, `Community 23`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
...
- **Are the 347 inferred relationships involving `WorldModel` (e.g. with `EngagementRefused` and `EngagementResult`) actually correct?**
  _`WorldModel` has 347 INFERRED edges - model-reasoned connections that need verification._
- **Are the 351 inferred relationships involving `NodeKind` (e.g. with `EngagementRefused` and `EngagementResult`) actually correct?**
  _`NodeKind` has 351 INFERRED edges - model-reasoned connections that need verification._